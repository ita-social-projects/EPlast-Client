...
const actions = {
  handleGetPrecautionTable: (): Action<State> => async ({
    setState,
    getState,
    dispatch,
  }) => {
  ...
  setVisibleAddModal: (visible: boolean): Action<State> => ({ setState }) => {
    setState({
      visibleModal: visible,
    });
  },

  setPageX: (x: number): Action<State> => ({ setState }) => {
    setState({
      pageX: x,
    });
  },

  setPageY: (y: number): Action<State> => ({ setState }) => {
    setState({
      pageY: y,
    });
  },

  setUserId: (id: any): Action<State> => ({ setState }) => {
    setState({
      userId: id,
    });
  },

  setRecordObj: (object: UserPrecautionTableItem): Action<State> => ({
    setState,
  }) => {
    setState({
      recordObj: object,
  ...
  handleAdd: (): Action<State> => async ({ setState, dispatch }) => {
    setState({
      visibleModal: false,
    });
  ...
  handleClickAway: (): Action<State> => ({ setState }) => {
    setState({
      showDropdown: false,
      selectedRow: -1,
    });
  ...
  setShowDropdown: (status: boolean, recordIndex: number): Action<State> => ({
    setState,
  }) => {
    setState({
      showDropdown: status,
  ...
  handleSearchPrecautionTable: (event: any): Action<State> => ({
    setState,
  }) => {
    setState({
      searchedData: event,
  ...
  showModalPrecautionTable: (): Action<State> => ({ setState }) => {
    setState({
      visibleModal: true,
    });
  },
  ...
  handleDeletePrecautionTable: (id: number): Action<State> => ({
    setState,
    getState,
    dispatch,
  }) => {
    const filteredData = getState().tableData.userPrecautions.filter(
      (d: { id: number }) => d.id !== id
    );
    ...
    if (
      getState().page != 1 &&
      getState().tableData.userPrecautions.length == 1
    ) {
      setState({
  ...
  handleEditPrecautionTable: (
    ...
    user: any,
    userId: string
  ): Action<State> => ({ setState, getState, dispatch }) => {
  ...
  tableSettings: (res: any): Action<State> => ({ setState }) => {
    setState({
      page: res[0].current,
    ...
    res[1].date === null
      ? setState({ dateSorter: [] })
      : setState({ dateSorter: res[1].date });
  ...
  FormAddPrecaution: (
    newPrecaution: UserPrecaution,
    form: any
  ): Action<State> => async ({ dispatch, setState }) => {
  ...
  addModalHandleSubmit: (values: any, form: any): Action<State> => async ({
    dispatch,
  }) => {
  ...
  addModalhandleCancel: (form: any): Action<State> => ({ dispatch }) => {
    form.resetFields();
    dispatch(actions.setVisibleAddModal(false));
  },

  fetchDataFormAddPrecaution: (): Action<State> => async ({ setState }) => {
    setState({
  ...
  dropDownHandleItemClick: (item: any, onDelete: any): Action<State> => async ({
    dispatch,
    getState,
  }) => {
    switch (item.key) {
  ...
  setUserPrecaution: (precaution: UserPrecaution): Action<State> => ({
    setState,
  }) => {
    setState({
  ...
  setShowEditModal: (status: boolean): Action<State> => ({ setState }) => {
    setState({
      showEditModal: status,
    });
  },

  setEditModalLoading: (status: boolean): Action<State> => ({ setState }) => {
    setState({
      editModalLoading: status,
    });
  },

  dropDownFetchData: (): Action<State> => async ({ getState, dispatch }) => {
    await precautionApi
      .getUserPrecautionById(getState().recordObj.id)
      .then((res) => dispatch(actions.setUserPrecaution(res.data)));
  },

  setEditModalPrecaution: (precaution: Precaution): Action<State> => ({
    setState,
  }) => {
    setState({
      editModalPrecaution: precaution,
  ...
  setEditModalUser: (newUser: any): Action<State> => ({ setState }) => {
    setState({
      editModalUser: newUser,
    });
  },

  editModalHandleCancel: (form: any): Action<State> => ({ setState }) => {
    form.resetFields();
    setState({
      showEditModal: false,
    });
  ...
  editModalSetPrecautionChange: (dist: any): Action<State> => ({
    setState,
  }) => {
  ...
  editModalSetUserChange: (user: any): Action<State> => ({ setState }) => {
  ...
  editModalHandleFinish: (
    editedUserPrecaution: any,
    form: any
  ): Action<State> => async ({ setState, getState, dispatch }) => {
    ...
    form.resetFields();
    const updatedUserPrecaution = (
      await precautionApi.getUserPrecautionById(newPrecaution.id)
    ).data;
    dispatch(
      actions.handleEditPrecautionTable(
  ...
  editModalFetchData: (): Action<State> => async ({ setState }) => {
    setState({
      editModalDistData: [],
      editModalUserData: [],
    ...
    await precautionApi
      ...
      .catch(() => {
        notificationLogic(
          "error",
          failGetAction("пересторог. Спробуйте пізніше")
        );
      });
  ...
  CreateDeleteNotification: (id: number): Action<State> => ({ getState }) => {
