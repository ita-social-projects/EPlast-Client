...
const getUserRenewalsTableData = async (searchedData: string, page: number, pageSize: number) => {
    return (await api.get('UserRenewal/UserRenewalsForTable',
        {
...
const renewUser = async (data: UserRenewal) => {
    const response = await api.put("UserRenewal/RenewUser", data)
        .then((response) => {
            toggleMemberStatus(response.data.id);
        })
        .catch(() => {
            showUserFormerInfoModal(
                "Неможливо відновити користувача."
            )
        });
    return response;
};
...
const checkFormer = async (email: string) => {
    let formerId: string = "";
    await api.post(`UserRenewal/FormerCheck/${email}`)
        .then((response) => {
