...
const doesRegisterNumberExist = async (numberInRegister: number) => {
  const response = await Api.get(
    `EducatorsStaff/registerexist/${numberInRegister}`
  );
  return response;
};
...
const doesRegisterNumberExistEdit = async (
  numberInRegister: number,
  kadraId: number
) => {
  const response = await Api.get(
    `EducatorsStaff/edit/registerexist/${kadraId}/${numberInRegister}`
  );
  return response;
};
...
const findUserByEduStaff = async (EduStaffId: number) => {
  const response = await Api.get(
    `EducatorsStaff/findUserForRedirect/${EduStaffId}`
  );
  return response;
};
...
const getEducatorsStaffForTable = async (
  newTableSettings: KadraTableSettings
) => {
  return (
    await Api.get(
      `EducatorsStaff/EducatorsStaffForTable`,
      newTableSettings,
      (params: any) => {
