...
export const addExtractFromUPU = async (userId: string, data: any) => {
  return api
    .post(`Blanks/AddExtractFromUPUDocument/${userId}`, data)
    .catch((error) => {
      throw new Error(error);
    });
};
...
export const removeDocument = async (documentId: number) => {
  return api
    .remove(`Blanks/RemoveBiographyDocument/${documentId}`, documentId)
    .catch((error) => {
      throw new Error(error);
    });
};
...
export const removeExtractFromUPUDocument = async (documentId: number) => {
  return api
    .remove(`Blanks/RemoveExtractFromUPUDocument/${documentId}`, documentId)
    .catch((error) => {
      throw new Error(error);
    });
};
...
const openFile = async (base64: string, format: string) => {
  if (
    ...
    pdfWindow?.document.write(
      "<iframe width='99%' height='99%' src='data:application/pdf;base64," +
        encodeURI(base64) +
        "'></iframe>"
    );
  }
...
export const openGenerationFile = async (userId: string) => {
  const response = await (
    await api.get(`Blanks/getGenerationFile/${userId}`, userId)
  ).data;
...
export const openExtractFromUPUFile = async (
  fileBlob: string,
  fileName: string
) => {
  const response = await (
...
export const openAchievemetFile = async (
  fileBlob: string,
  fileName: string
) => {
  const response = await (
...
export const getAchievementFile = async (
  fileBlob: string,
  fileName: string
) => {
  const response = await (
...
export const getExtractFromUPUFile = async (
  fileBlob: string,
  fileName: string
) => {
  const response = await (
