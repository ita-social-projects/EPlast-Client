...
export default function ClubAnnualReportLayout(annualReport: ClubAnnualReport) {
  var adminFirstLastName = `${annualReport.head?.user?.firstName ?? ""} ${
    annualReport.head?.user?.lastName ?? ""
  }`;
  ...
  return {
    ...
    content: [
      ...
      {
        text: `Звіт куреня ${annualReport.clubName} за ${new Date(
          annualReport.date
        ).getUTCFullYear()} рік`,
        style: "header",
      },
            ...
            table: {
              ...
              body: [
                ...
                ...annualReport.admins?.map((element) => [
                  {
                    text: `${
                      `${element.user.firstName} ${element.user.lastName}`.replace(
                        / /g,
                        ""
                      ).length > 0
