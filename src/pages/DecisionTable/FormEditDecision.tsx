...
const FormEditDecision = ({
  ...
  const fetchUser = async () => {
    ...
    setCanEdit(
      roles.includes(Roles.RegionBoardHead) || roles.includes(Roles.Admin)
    );
  };
  ...
  const handleFinish = async (dec: any) => {
    ...
    const decisionStatusName = canEdit
      ? dec.decisionStatusType
      : statusTypeGetParser(decision.decisionStatusType);
    user = jwt(curToken);
