...
const AnnualReportEdit = () => {
  ...
  const fetchAnnualReport = async () => {
    try {
      let annualreport = await AnnualReportApi.getAnnualReportEditFormById(
        id
      ).then((response) => {
        return response.data.annualReport as AnnualReport;
      });
      if (annualreport.city) {
        setTitle(title.concat(" ", annualreport.city.name));
        setCityMembers(
          (annualreport.city.cityMembers as []).map((item: any) => {
            return {
  ...
  const handleFinish = async (obj: any) => {
    ...
    let membersStatistic = Object.assign(
      annualReport?.membersStatistic,
      obj.membersStatistic
    );
