...
const AnnualReportForm = (props: Props) => {
  ...
  const validationSchema = {
    ...
    money: [
      { required: true, message: emptyInput() },
      { pattern: /^[1-9]\d*(\.\d+)?$/, message: shouldContain("цілі додатні десяткові числа у форматі 0.00") },
      {
        validator: (_: object, value: string) =>
              ...
              >
                <Input
                  ...
                  onChange={(e) => {
                    formHook.setFieldsValue({
                      membersStatistic: {
                        numberOfSeniorPlastynSupporters: getOnlyNums(
                          e.target.value
                        ),
                      },
                    });
              ...
              >
                <Input
                  ...
                  onChange={(e) => {
                    formHook.setFieldsValue({
                      membersStatistic: {
                        numberOfSeniorPlastynMembers: getOnlyNums(
                          e.target.value
                        ),
                      },
                    });
