...
export default function () {
  ...
  const sendRenewal = async (data: any) => {
    await userRenewalsApi
      .sendUserRenewal(data)
      .then(() => {
      ...
      .catch((error) => {
        if (error.response.data == "Cannot create renewal") {
          showUserFormerInfoModal(
            `Неможливо надіслати запит! Зважте, що запит 
                    можна надсилати не частіше, ніж один раз на 10 днів!`,
            "/signin"
          );
        } else {
          showUserFormerInfoModal(
            "Сталася помилка при надсиланні запиту. Спробуйте пізніше.",
            "/signin"
          );
        }
      });
      ...
      content: (
        <div>
          ...
          <div style={{ textAlign: "center" }}>
            <h3>
              Введіть адресу електронної пошти, за якою ви мали доступ до сайту,
              щоб підтвердити свій статус.
            </h3>
          </div>
          <Form name="UserRenewalForm" form={form} onFinish={handleCheckFormer}>
    ...
    },
    {
      ...
      content: (
        <div>
          ...
          <Form name="UserRenewalForm" form={form} onFinish={handleCityChange}>
            <Form.Item
              name="CityId"
    ...
    },
    {
      ...
      content: (
        <div>
          ...
          <div style={{ textAlign: "center" }}>
            <h3>
              Ви надсилаєте запит на відновлення статусу у Пласті! Процедура
              відновлення може тривати до 10 днів. Протягом цього терміну
              надіслати новий запит буде неможливо!
            </h3>
          </div>
          <Form name="UserRenewalForm" form={form} onFinish={handleSendRenewal}>
            <Button htmlType="submit" id={styles.confirmButton}>
              Надіслати запит
            </Button>
          </Form>
