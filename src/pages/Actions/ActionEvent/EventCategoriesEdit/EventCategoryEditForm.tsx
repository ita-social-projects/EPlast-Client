...
export const EventCategoryEditForm: React.FC<EventCategoryEditFormProps> = ({
  ...
  const onFinish = (values: any) => {
    ...
    eventsApi
      .updateEventCategory(editedCategory)
      .then(() => {
        const categoryToUpdateIdx = categories.findIndex(
          (c) => c.eventCategoryId === editedCategory.eventCategoryId
        );
  ...
  return (
    <Form form={form} onFinish={onFinish} layout="inline" size="large">
      <Input.Group compact>
        ...
        >
          <Input placeholder="Редагувати категорію" />
        </Form.Item>
        <Form.Item noStyle>
