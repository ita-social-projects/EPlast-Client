...
const Sectors = () => {
  ...
  const getSectors = async () => {
    setLoading(true);
    const responseSectors = await getSectorsListByGoverningBodyId(governingBodyId);
            ...
            sectors.map(sector => (
              ...
              >
                <div className="cityMember">
                  <div
                    onClick={() => history.push(`/governingBodies/${governingBodyId}/sectors/${sector.id}`)}
                  >
                    {photosLoading ? (
                      <Skeleton.Avatar active size={86}></Skeleton.Avatar>
                    ) : (
                      <Avatar size={86} src={sector.logo === null ? undefined : sector.logo} />
                    )}
                    <Card.Meta
