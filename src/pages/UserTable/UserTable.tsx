...
const UsersTable = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setCurrentUser(
                (await userApi.getUserProfileById(user.nameid, user.nameid))
                    .data.user
            );
    ...
    const ondeSelect = (selectedKeys: any, e: any) => {
        if (e.value == 0) {
            ...
            setDynamicCities((prev) =>
                prev.filter(
                    (item) => item !== (e.value.split(" ")[1] as number)
                )
            );
            ...
            setDynamicDegrees((prev) =>
                prev.filter((item) => e.value.split(" ")[1] as number)
            );
