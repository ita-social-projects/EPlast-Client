...
const AddAdministratorModal = (props: Props) => {
  ...
  const getCityAdmins = async () => {
    ...
    if (props.cityId !== 0) {
      ...
      const Head = (responseAdmins.data as CityAdmin[]).find(
        (x) => x.adminType.adminTypeName === Roles.CityHead
      );
      setHead(Head);
      const HeadDeputy = (responseAdmins.data as CityAdmin[]).find(
        (x) => x.adminType.adminTypeName === Roles.CityHeadDeputy
      );
        ...
        >
          <AutoComplete
            ...
            options={[
              ...
              { value: Roles.CityReferentUPS },
              { value: Roles.CityReferentUSP },
              { value: Roles.CityReferentOfActiveMembership },
              { value: "Голова СПР" },
              { value: "Писар" },
              { value: "Скарбник" },
              { value: "Фотограф" },
