...
const FormAddPlastDegree = ({
  ...
  const handleOnChange = (value: any) => {
    if (value === "Пласт прият") {
      setFiltredDegrees(
        availablePlastDegree.filter((item) => item.name === "Пласт прият")
      );
      setIsDisable(true);
      setIsRequired(false);
  ...
  return (
    <Form name="basic" onFinish={handleFinish} form={form}>
      ...
      <Form.Item
        name="plastDegree"
        rules={[{ required: isRequired, message: emptyInput() }]}
      >
        <Select placeholder={"Оберіть ступінь"} disabled={isDisable}>
