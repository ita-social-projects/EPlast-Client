...
import activeMembershipApi, { UserDates, UserPlastDegree } from '../../../api/activeMembershipApi';
        ...
        <div className={classes.line} />
            {
                ...
                <>
                    <div className={classes.textGeneralInfo}>
                        Дата вступу: { dates?.dateEntry === "" ? 'Не задано' : moment(dates.dateEntry).format("DD-MM-YYYY")}
                    </div>
                    <div className={classes.textGeneralInfo}>
                        Дата присяги: {dates?.dateOath === "" ? 'Без присяги' : moment(dates.dateOath).format("DD-MM-YYYY") } 
                    </div>
                    <div className={classes.textGeneralInfo}>
                        Дата завершення: {dates?.dateEnd === "" ? 'Ще в Пласті' : moment(dates.dateEnd).format("DD-MM-YYYY") } 
                    </div>
                    {IsUserHasAnyAdminTypeRoles(userToken["http://schemas.microsoft.com/ws/2008/06/identity/claims/role"])
                    &&
                    <Button type="primary"className={classes.buttonChange}  onClick={() => {setDatesVisibleModal(true)}}>
                        Змінити
                    </Button>
                    }
                 </>
