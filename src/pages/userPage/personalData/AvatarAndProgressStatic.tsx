...
const AvatarAndProgressStatic: React.FC<AvatarAndProgressStaticProps> = (
  ...
  return loading === false ? (
    ...
    <div className="kadraWrapper">
      ...
      <p className="statusText">Станиця: <Link to="#" className="LinkText" onClick={() => history.push(`/cities/${cityId}`)}>{city}</Link></p>
      <p className="statusText">Курінь: <Link to="#" className="LinkText" onClick={() => history.push(`/clubs/${clubId}`)}>{club}</Link></p>
      {!isUserPlastun && (
