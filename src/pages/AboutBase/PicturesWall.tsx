...
const PicturesWall = (subsectionId: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await aboutBase.getSubsectionPictures(subsectionId.subsectionId);
  ...
  return (
    <Row justify='start' style={{ marginBottom: '15px' }}>
      <Col span={16} style={{ overflow: 'auto' }}>
        <Row justify='start'>
          {
            pictures.map(picture => <Col xs={24} md={12} lg={6} key={picture.picturesId}>
              <Card
                hoverable
                cover={<img style={{ height: '130px' }} alt="example"
                  src={picture.fileName} />}
                actions={RenderPictureWallActions(picture.picturesId, picture.fileName)}
