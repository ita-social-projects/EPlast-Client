...
const PicturesWallEdit = (subsectionId: any) => {
  ...
  const removePicture = (pictureId: number) =>
    setPictures(pictures.filter((picture) => picture.pictureId !== pictureId));
  ...
  const fetchData = async () => {
    const response = (
      await aboutBase.getSubsectionPictures(subsectionId.subsectionId)
    ).data;
  ...
  const RenderPictureWallActions = (
    ...
    pictureActions.push(
      <DeleteOutlined
        ...
        key="deletePicture"
        onClick={() => {
          deletePicture(id).then(() => seeDeleteModal(id));
        }}
      />
    );
      ...
      <Col span={24} style={{ overflow: "auto" }}>
        <Row justify="start">
          {pictures.map((picture) => (
            <Col xs={12} sm={12} key={picture.pictureId}>
              <Card
                hoverable
                cover={
                  <img
